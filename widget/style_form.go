...
// errStyleStyleFormItemInitialState defines the error if the initial validation for a StyleFormItem result
// in an error
var errStyleStyleFormItemInitialState = errors.New("widget.StyleFormItem initial state error")

// StyleFormItem provides the details for a row in a form
type StyleFormItem struct {
	Text   string
...
// NewStyleFormItem creates a new form item with the specified label text and input widget
func NewStyleFormItem(text string, widget fyne.CanvasObject) *StyleFormItem {
	return &StyleFormItem{Text: text, Widget: widget}
}

var _ fyne.Validatable = (*StyleForm)(nil)

// StyleForm widget is two column grid where each row has a label and a widget (usually an input).
...
type StyleForm struct {
	...
	ButtonLayout  func(*Button, *Button) *fyne.Container
	ContentLayout func(*fyne.Container, *fyne.Container) *fyne.Container
...
func (f *StyleForm) Append(text string, widget fyne.CanvasObject) {
	item := &StyleFormItem{Text: text, Widget: widget}
	f.AppendItem(item)
}

// AppendItem adds the specified row to the end of the Form
func (f *StyleForm) AppendItem(item *StyleFormItem) {
...
func (f *StyleForm) createInput(item *StyleFormItem) fyne.CanvasObject {
	...
	return &fyne.Container{Layout: styleFormItemLayout{form: f}, Objects: []fyne.CanvasObject{item.Widget, textContainer}}
}

func (f *StyleForm) itemWidgetHasValidator(w fyne.CanvasObject) bool {
...
func (f *StyleForm) createLabel(text string) fyne.CanvasObject {
	th := f.Theme()
	v := fyne.CurrentApp().Settings().ThemeVariant()
	...
	return &fyne.Container{Layout: &styleFormLabelLayout{form: f}, Objects: []fyne.CanvasObject{label}}
}

func (f *StyleForm) updateButtons() {
	if f.CancelText == "" {
		f.CancelText = "Cancel"
	}
	if f.SubmitText == "" {
...
func (f *StyleForm) checkValidation(err error) {
	if err != nil {
		f.submitButton.Disable()
		return
	}
...
func (f *StyleForm) ensureRenderItems() {
...
func (f *StyleForm) isVertical() bool {
...
func (f *StyleForm) setUpValidation(widget fyne.CanvasObject, i int) {
	updateValidation := func(err error) {
		if err == errStyleStyleFormItemInitialState {
			return
		}
	...
	if w, ok := widget.(fyne.Validatable); ok {
		...
		if e, ok := w.(*Entry); ok {
			...
			if e.Validator != nil && f.Items[i].invalid {
				...
				e.SetValidationError(errStyleStyleFormItemInitialState)
			}
		}
...
func (f *StyleForm) setValidationError(err error) {
...
func (f *StyleForm) updateHelperText(item *StyleFormItem) {
	th := f.Theme()
	v := fyne.CurrentApp().Settings().ThemeVariant()
...
func (f *StyleForm) CreateRenderer() fyne.WidgetRenderer {
	f.ExtendBaseWidget(f)
	th := f.Theme()
	...
	if f.ButtonLayout != nil {
		f.buttonBox = f.ButtonLayout(f.cancelButton, f.submitButton)
	} else {
	...
	f.validationError = errStyleStyleFormItemInitialState // set initial state error to guarantee next error (if triggers) is always different
	...
	if f.ContentLayout != nil {
		content = f.ContentLayout(f.itemGrid, f.buttonBox)
	} else {
...
// NewStyleForm creates a new form widget with the specified rows of form items
// and (if any of them should be shown) a form controls row at the bottom
func NewStyleForm(items ...*StyleFormItem) *StyleForm {
	form := &StyleForm{Items: items}
	form.ExtendBaseWidget(form)

	return form
}

type styleFormLabelLayout struct {
	form *StyleForm
}

func (f styleFormLabelLayout) Layout(objs []fyne.CanvasObject, size fyne.Size) {
...
func (f styleFormLabelLayout) MinSize(objs []fyne.CanvasObject) fyne.Size {
	innerPad := f.form.Theme().Size(theme.SizeNameInnerPadding)
	min0 := objs[0].MinSize()
...
type styleFormItemLayout struct {
	form *StyleForm
}

func (f styleFormItemLayout) Layout(objs []fyne.CanvasObject, size fyne.Size) {
	innerPad := f.form.Theme().Size(theme.SizeNameInnerPadding)
...
func (f styleFormItemLayout) MinSize(objs []fyne.CanvasObject) fyne.Size {
	innerPad := f.form.Theme().Size(theme.SizeNameInnerPadding)
	min0 := objs[0].MinSize()
	min1 := objs[1].MinSize()
